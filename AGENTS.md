...
Both names go on the record kind, `tasks-axi add <id> "<title>" --kind fog|out-of-course`, which is the only field the chart classifies by; then `tasks-axi hold <id> --reason "<why>" --kind future` records the reason the chart prints, because `hold --kind` is a separate closed vocabulary that rejects both names.
