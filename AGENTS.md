...
This machine now has swap as a shock absorber, and this seat is expected to move behind a memory limit, so the alarm is no longer the only boundary between runaway growth and a kernel kill.
