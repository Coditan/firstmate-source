...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  findings/          the political officer's findings surface when no config/findings-dir points elsewhere: one append-only JSON record per finding, written by bin/fm-finding.sh and never edited afterwards (docs/findings-surface.md)
