...
| `fm-transcript-reduce.py` | Build this home's reduced, redacted session derivative with one of two readers selected by `--source`, or re-verify an existing one to zero; refuses a source that disagrees with the material's shape rather than writing the empty archive a wrong reader would produce (docs/session-archive.md) |
| `fm-transcript-search.sh` | Search that derivative with context, narrowing the file set by date and working directory first; grep is the index, so there is none to go stale, and an archive that is not there reads as absent rather than as no matches (docs/session-archive.md) |
| `fm-transcript-refresh.sh` | Rebuild both stores from this vessel's own raw transcripts, verify the output to zero, and leave the honest bound on the artefact; the raw stores are read-only inputs and a store this vessel does not have is skipped, never counted as empty (docs/session-archive.md) |
