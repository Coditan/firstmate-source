...
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, then atomically arm a static merge poll; `--no-watch` records without arming and says no watch exists; `--disarm <id>` retires an armed poll, removing the whole artifact set and leaving the recorded `pr=`, and refuses when the task's check is a registered custom check rather than a merge poll |
