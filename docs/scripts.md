...
| `fm-tg-send.sh`          | Send the captain one message on the direct Telegram channel, refusing loudly rather than reporting a delivery nobody got |
...
| `fm-journal-lib.sh`      | The event journal's record contract: every notification event written once, in arrival order, carrying the state it referred to as that state read at arrival, under a stated size bound |
