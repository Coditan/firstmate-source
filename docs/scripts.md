...
| `fm-sessionstart-nudge.sh` | Record the primary session's transcript position, then print the native session-start hook nudge when it has not already run the digest; with `--rebind-to-lock`, rebind that record to the holder this home's session lock now names |
...
| `fm-status.sh`           | Compose and append one status line for a task from its parts, refusing by name a verb, key, note, or path the readers could not use |
...
| `fm-memory-alarm.sh`     | Wake the fleet on three conditions - RAM headroom below a floor, growth that would exhaust that headroom inside a horizon, and memory stall held continuously past a window, the last being the only one that can see a machine already drowning in swap rather than about to run out - naming the process responsible with its account and the work it serves; it reads the attribution reading and nothing else, sets no limit and kills nothing, and reports an instrument it could not read as blindness rather than an all-clear - an input no condition needs leaves the rest still judged, and only an unreadable RAM headroom leaves nothing judged at all (docs/memory-alarm.md) |
...
| `fm-toolbelt-domain-map.py` | Generate and check the evidence map of top-level `bin/` commands, their domains, purposes, and callers |
...
| `fm-reachability-lib.sh` | Own a service's reachability verdict and the evidence behind it, refusing any claim of more reach than something established (docs/lavish-access.md) |
| `fm-tailnet-cli-lib.sh`  | Own how this vessel's `tailscale` client reaches its own daemon, reading the socket path the image declares rather than writing one down (docs/lavish-access.md) |
...
| `fm-absence-lib.sh`      | Shared rule for whether a path's ABSENCE was established or is merely what a failed stat reports, used by fleet-sync and bootstrap |
...
| `fm-check-lib.sh`        | Validate custom-check registrations, prepare private execution snapshots, and refuse an arm whose state directory belongs to another home |
...
| `fm-skills-lock.sh`     | Report whether this seat carries the third-party plugin skills the fleet expects, naming the command an operator would run and running none of it (docs/fleet-plugin-skills.md) |
