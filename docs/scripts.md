...
| `fm-tmux-lib.sh`         | Shared tmux pane primitives for own-window startup repair, busy detection, composer capture, and verified submit |
...
| `fm-pr-merge.sh`         | Record PR metadata, then merge a task's canonical full GitHub URL and have the forge delete the merged head branch |
