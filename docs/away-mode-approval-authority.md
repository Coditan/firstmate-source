...
- **The absolute part of the list did not move.**
  ...
  Ask-user findings sit with merges rather than with those three: section 7 places them with the captain wherever `yolo` is off and with firstmate wherever it is on [later note: under either posture it now carries one standing exception, a correction that only brings a claim back to what its own run measured inside the module already under change, which never reaches the captain; `ask-user-authority` owns that boundary], so stating them as an absolute wait would have withdrawn an authority granted elsewhere in exactly the way the merge item did.
