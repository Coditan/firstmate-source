...
The shared target-existence probe reports absence only from completed workspace, surface, and expected-label inventories, reports unreadable when any inventory command or JSON parse fails, and treats a replacement surface in the correctly labelled workspace as present.
