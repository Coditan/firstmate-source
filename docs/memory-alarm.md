...
`bin/fm-memory-alarm.sh` wakes the fleet when `hlr-web-1` is running out of memory and names the process responsible, its account, and the work it was serving.
...
The captain's decision after that measurement was to build the alarm on headroom and growth with no cgroup limit at all.
Swap is being tracked separately as host work; it is not a precondition of this alarm, and it would not make a ceiling viable.
...
| --------- | ------------ | -------------- |
| headroom | available memory is below the floor | a backstop, for memory going somewhere no tracked process is visibly growing into |
| horizon | total growth across tracked processes would consume all remaining memory within the horizon | the primary trigger, and the one that warns rather than confirms |
...
The horizon self-tightens as memory goes, which is what lets one number work across the whole range.
...
The horizon is **three times that cadence**, so a crossing is seen at least twice before the memory it predicts is gone.
A horizon at or below the sweep interval could take the machine from silent to out-of-memory between two polls without ever firing.
...
10.2% of this machine's memory, and **6.1 times below** the lowest headroom ordinary busy operation reached.
...
Driven on 2026-08-13 by a process allocating at roughly 20 MiB/s to a hard 2,200 MiB cap, with 16 GB available on the host throughout:

```
--- poll A (first sighting) ---
(silent)
--- poll B ---
MEMORY_ALARM: this machine is running out of memory - growth across the running work
totals 1185 MiB/min, which would use up the 14628 MiB still available in about 12.3
minutes. Largest grower: python3 balloon.py (pid 1256767), account coditan, serving
task fleet-host-protection-after-reaper-panel-memory-ceiling-alarm (ship,
firstmate-fork), growing 1184 MiB/min. Nothing has been limited or killed.
--- poll C (still crossed) ---
(silent)
--- poll D (runaway gone) ---
MEMORY_ALARM: recovered - 16080 MiB available, growth 8 MiB/min. The shortage lasted 2m28s.
--- poll E ---
(silent)
```
...
```
2026-08-13T20:10:39Z  ok -> crossed  14628 MiB available  1185 MiB/min growth  12.3 minutes left  python3 balloon.py (pid 1256767), account coditan, serving task ... (ship, firstmate-fork), growing 1184 MiB/min
2026-08-13T20:13:07Z  crossed -> ok  16080 MiB available     8 MiB/min growth               ...
```

`tests/fm-memory-alarm-crossing-e2e.test.sh` is that proof as a test: it drives a real runaway, sized from the host's own available memory rather than a fixed number, and requires the alarm to fire, name the process and account, record both transitions, and leave the process running.
...
```
  silent avail=16037 MiB load=1.99  memory-alarm: ok - 16041 MiB available, growth 0 MiB/min
  silent avail=16047 MiB load=2.59  memory-alarm: ok - 16049 MiB available, growth 24 MiB/min
  silent avail=16083 MiB load=2.50  memory-alarm: ok - 16095 MiB available, growth 1 MiB/min
  silent avail=15998 MiB load=2.35  memory-alarm: ok - 15999 MiB available, growth 20 MiB/min
  ...
  silent avail=16105 MiB load=2.41  memory-alarm: ok - 16102 MiB available, growth 3 MiB/min
  silent avail=16104 MiB load=2.20  memory-alarm: ok - 16102 MiB available, growth 10 MiB/min

RESULT: the alarm stayed silent through the whole busy period
```

Across those polls: available memory 15,998-16,117 MiB, total growth 0-24 MiB/min.
