...
- **Nothing established either way** - `reachability=untested`, which several different runs produce, and the wrapper says a different sentence for each because only one of them can be settled by trying again.
  ...
  Every later run on the unreadable-status host returns the same non-answer, so the first sentence's closing promise would be untrue there; `bin/fm-service-port.sh`'s header owns which run resolves which value.
