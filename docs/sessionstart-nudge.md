...
Each rebind sweeps two kinds of stash it can prove will never be promoted again: one older than pid 1's start, because it belongs to a pid table that no longer exists, and one whose named pid is no longer live, because promotion needs that pid's incarnation to be readable now and to match, which a process that is gone can never satisfy.
The second reading is what bounds the accumulation this state could otherwise cause: a helper session the harness starts in the primary's own cwd is refused the record, stashes, and exits without ever taking the lock, and its file would otherwise sit in `state/` for the life of the container.
...
There a record naming a holder that is alive right now is left exactly as it is rather than destroyed, and the stash sweep stands down entirely, because the alternative is an unmeasured ceiling on every session of that host and no promotable stash left to fix it with.
