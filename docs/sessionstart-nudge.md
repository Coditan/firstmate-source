...
| `error` | on `status=error` only, the cause: `no-hook-payload`, `no-transcript-path`, `no-session-id`, `unusable-transcript-path`, `unusable-session-id`, `no-jq`, `no-harness-process`, `harness-lookup-failed`, or `rebound-without-hook-payload` |
...
A session start that finds another live harness holding `state/.lock` leaves the record alone - it neither replaces nor removes it - and still prints the nudge, because a lock-refused session must still run session start to discover that it is read-only.
It does not walk away empty-handed either: it stashes its own payload beside the record, for the rebind described at the end of this section, and that stash is never the record.
...
There is one rebind implementation and one call, and it sits under `publish_record` in `bin/fm-lock.sh`, the single writer of the lock record.
Every acquisition outcome therefore rebinds by construction, the plain acquisition and the dead-container supersede and the handover redemption alike, including any acquisition path added later, and a seat that runs `bin/fm-lock.sh` directly rebinds exactly as one that runs session start does.
Publishing is not always acquiring, so the call is reached on acquisition only: the handover offer rewrites the record to add a ticket while the same seat stays the holder, and there is no new holder to rebind to.
Each acquiring path prints what the rebind did beside its own acquisition line, and `bin/fm-session-start.sh` shows that text because it prints the lock script's output; only the printing is per-path, and a path that printed nothing would still have rebound the record.
The rebind promotes what the refused hook kept, and it tries that FIRST, before asking anything about the record already standing.
A stash and a record naming the same process are not equal claims: the stash is always the newer of the two, because every hook run that publishes the record clears the stashes it owns before writing and promotion clears the stash before publishing.
So a record naming this holder can be one an earlier session wrote in this same process, which a clear leaves behind, and letting it win would measure this session's ceiling against that session's transcript with nothing said.
Only when nothing can be promoted does what is standing decide: a record that already names the holder is then left alone when its mtime is not older than pid 1's start, and also when this host cannot read that start at all and the holder is alive, for the reason set out below.
...
A refused run that yielded no usable payload, or whose own incarnation cannot be read, removes its own stash and leaves nothing behind that a later rebind could read as this session's; when it cannot name that stash at all it clears the wider set described below instead.
The rebind reads only the stash named by the holder the lock now publishes, and promotes it as the record when its `harness_pid` names that holder and its `harness_incarnation` is the incarnation that pid is running under at this moment.
Promotion asks the incarnation first and treats it as decisive, and it does not consult the container-age reading at all.
Those two readings can disagree about one file, because the age reading is a wall-clock comparison against `btime` while the incarnation is a process fact, and a live process cannot be older than the container it is running in.
So when they disagree the clock is the reading that is wrong, and refusing on it would also delete the only copy of this session's transcript position, leaving the ceiling unmeasured for the life of the session with nothing left to repair it from.
The age reading still runs in the sweep, where it only removes what no live process vouches for.
Pid and incarnation are one proof in two halves, and the second half is what closes pid reuse INSIDE one container: a session refused the record can exit without ever taking the lock, the number it named is handed to the next harness session in that same container, and its stash survives the sweep for exactly that reason, because that number is live again.
What neither half can close is two SESSIONS inside one harness PROCESS, which is what a clear makes: a stash the previous session left passes every process-level proof there is, because the process really did write it and really is still alive.
That one is closed where it arises rather than at promotion, because the rebind has no session id of its own to compare against - only the SessionStart payload carries one, and the rebind runs in `bin/fm-lock.sh` with no payload at all.
So the invariant is held at the writing end: every hook run in a process leaves that process's stash describing itself or leaves none, and a run that cannot resolve its own harness pid has no filename to speak for it and must not leave one standing as its own.
How wide that clearing has to be depends on which failure the run hit, and the two are kept apart because both are measured rather than hypothetical.
On `no-harness-process` the ancestry walk COMPLETED and simply found no harness among the processes it read, so the run does know which processes it runs under, and only a stash naming one of those could be confused with its own: `bin/fm-harness-pid-lib.sh` publishes that walk as `FM_HARNESS_PID_ANCESTRY` and only those stashes are cleared.
On `harness-lookup-failed` the process table could not be read at all, the run knows nothing about its own ancestry, and every stash is cleared.
The narrowing matters because the wide sweep run from an unrelated helper session would destroy the primary's only copy of its transcript position over a probe failure that had nothing to do with it.
Within the set it does clear, the trade is the right way round, because a discarded stash costs an unmeasured ceiling that says so while a stash promoted for the wrong session measures the ceiling against another session's transcript and says nothing.
...
Each rebind sweeps the files it can prove will never be used again, on two readings that hold on different hosts.
The liveness reading holds everywhere and needs nothing from `/proc`: a stash whose named pid is no longer live can never be promoted, because promotion needs that pid's incarnation to be readable now and to match, which a process that is gone can never satisfy.
That reading is what bounds the accumulation this state could otherwise cause: a helper session the harness starts in the primary's own cwd is refused the record, stashes, and exits without ever taking the lock, and its file would otherwise sit in `state/` for the life of the container.
It also clears the half-written temporaries the stash writer leaves if it dies between writing and renaming, which nothing else ever removes: those are named `<stash>.<writer pid>`, so a live writer's file is a rename still in flight and is left alone, while a dead writer's is an orphan no rename will ever claim.
The age reading needs this container's start and so holds only where that can be read: a stash older than pid 1's start belongs to a pid table that no longer exists.
It is never applied to a stash whose recorded incarnation is the one its pid is running under now, for the reason promotion gives: that file was written by a live process, which cannot predate the container it is running in, so a clock that calls it old is the reading to doubt and the file is that session's only copy.
...
The record's age is its file mtime, weighed against pid 1's start the same way the lock's dead-container predicate weighs it.
Those two readings give three answers rather than two, and only one of them condemns a record.
A reading that CAN be taken and shows the record predates this container replaces it, and a stash proven that way is swept.
A container start that cannot be read on this host at all proves nothing either way: `fm_container_start_epoch` reads `/proc/stat` and `/proc/1/stat`, so it answers nothing on Darwin, and nothing in a Linux container whose hidepid hides `/proc/1`.
There a record naming a holder that is alive right now is left exactly as it is rather than destroyed, once promotion has been tried and found nothing, and the age half of the stash sweep stands down, because the alternative is an unmeasured ceiling on every session of that host and no promotable stash left to fix it with.
A promotable stash still overrides such a record, because a stash whose recorded incarnation matches the live holder does tell this container's record from a previous container's even where the container start cannot be read.
The liveness half of the sweep still runs there, because it proves what it removes without asking when the container started.
The cost of that choice is stated rather than hidden: on such a host a record left by a previous container whose harness pid is live again here is kept, because nothing on that host can tell the two apart.
Promotion is unaffected by the unreadable reading, because the incarnation match is the stronger proof: the process that wrote the stash is alive and is still the same one, so the stash cannot have come from a container that has already gone.
When it did come to an error record, that record becomes `status=ok` again at the next SessionStart hook run in that session, such as a `/clear`, which carries the payload.
...
It proves the stash and the rebind together: a refused session keeps its own payload and the rebind promotes it into a measurable record, a stash naming another holder or the same pid in another incarnation is refused and the record becomes the explicit `rebound-without-hook-payload` error, a promotable stash wins over a record the same process wrote for an earlier session, a second refused session never destroys the primary's stash, and a run that cannot name its own harness leaves no stash standing as its own - narrowed to its own ancestry when the walk completed.
It proves the sweep's two readings apart: a stash whose process is gone is removed, one a live process vouches for survives a clock that calls it old, stashes predating this container are swept, and a host that cannot read its container start keeps the record naming its live holder and every usable stash while still promoting.
It also verifies tracked wrapper registration for Claude, Codex, OpenCode, Pi, and Grok.
`tests/fm-lock.test.sh` proves the rebind is reached from the lock record's own publisher rather than from any caller: a plain acquisition and a redeemed handover each rebind the record to the acquiring session, and a handover offer, which names no new holder, leaves it exactly as it is.
`tests/fm-session-start.test.sh` proves the end of the 2026-09-06 failure - an ordinary acquisition over a record naming a dead pid rebinds from the kept payload and the ceiling reads enforced again rather than as a session mismatch.
