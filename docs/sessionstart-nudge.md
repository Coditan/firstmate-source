...
A hook run that is refused the record still reads its payload and stashes it as `state/.primary-transcript.pending`, naming its own harness pid; the rebind publishes that stash as the record when it names the holder the lock now names and postdates this container, and discards it either way, so a stash is never promoted twice and a session that never took the lock can never hand its transcript to the one that did.
