...
A record that already names that holder is left alone when its mtime is not older than pid 1's start, and also when this host cannot read that start at all and the holder is alive, for the reason set out below.
...
So the invariant is held at the writing end: every hook run in a process leaves that process's stash describing itself or leaves none, and a run that cannot resolve its own harness pid has no filename to speak for it and therefore clears every stash rather than letting one stand as its own.
The cost of that sweep is a stash another live session might still have promoted, and it is the right way round, because a discarded stash costs an unmeasured ceiling that says so while a stash promoted for the wrong session measures the ceiling against another session's transcript and says nothing.
...
There a record naming a holder that is alive right now is left exactly as it is rather than destroyed, and the age half of the stash sweep stands down, because the alternative is an unmeasured ceiling on every session of that host and no promotable stash left to fix it with.
