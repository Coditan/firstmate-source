...
The rebind reads only the stash named by the holder the lock now publishes, and promotes it as the record when its `harness_pid` names that holder, its `harness_incarnation` matches that pid's incarnation as it reads now, and its mtime is not shown to predate this container.
