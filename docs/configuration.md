...
`bin/fm-check-lib.sh` owns that predicate, the refusal every `--arm` path and `bin/fm-check-register.sh` give, and the incident it closes; `tests/lib.sh` clears these four before any behavior test runs, for the same reason.
