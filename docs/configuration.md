...
The watcher breaks its check sweep at the first check that prints a line, so that id is named to sort before the alarm's `state/seat-vacancy.check.sh` and the alarm can never displace the convergence; both `--arm` paths retire their pre-rename predecessor shim and `.check-trust` by exact name once the replacement is registered.
