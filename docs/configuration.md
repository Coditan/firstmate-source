...
The seat alarm keeps `state/seat-alarm.check.sh` with its `.check-trust` for the armed watcher check, and `data/seat-alarm.log` and `data/seat-alarm.state` for its append-only history and for the last reading it took.
