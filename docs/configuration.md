...
The session-start secondmate liveness sweep uses a deeper `fm_backend_agent_alive` probe where verified, and the session-start digest's per-task endpoint line reads the same probe to say what is running behind a window that exists.
Today that probe can classify tmux and herdr endpoints as `alive`, `dead`, or `unknown`; zellij, Orca, and cmux report `unknown` until their own agent-process classifiers are verified, which is why a digest endpoint line on those three backends reads `unknown` rather than naming an agent.
...
`sandbox_mode` is the one value that is host-conditional on the launch line: on a host whose kernel cannot start a sandbox, the spawn launches `danger-full-access` instead and says so once, while the tracked file itself is never rewritten; [`docs/codex-sandbox-unavailable.md`](codex-sandbox-unavailable.md) owns that probe and its outcomes.
...
A Codex crewmate also receives its project's no-mistakes gate repository as a third `writable_roots` entry, resolved from a plain-path or local `file://` effective push destination while honoring `remote.<name>.pushurl` and `url.<base>.pushInsteadOf`, rejecting non-local schemes and scp-style destinations, and validating the directory as its own bare Git repository, so its pipeline push can create the quarantine directory git-receive-pack needs; a Codex secondmate does not receive it, and neither does the supervising session.
[`docs/codex-sandbox-gate-repo.md`](codex-sandbox-gate-repo.md) owns the measurements and the reason the grant names one gate rather than the repository root.
...
Graph freshness and navigation rules are owned by [`docs/graphify.md`](graphify.md).
...
Setting some of them and inheriting the rest is the one combination that is refused rather than resolved: arming a watcher check requires `FM_HOME` and the state directory to name ONE home, because an armed check bakes its locations and a mismatch installs one home's check into another home's `state/`.
`bin/fm-check-lib.sh` owns that predicate, the refusal every `--arm` path and `bin/fm-check-register.sh` give, and the incident it closes.
For the same reason `tests/lib.sh` clears all seven location variables before any behavior test runs - the four above plus `FM_HOME`, `FM_ROOT_OVERRIDE`, and `FM_FINDINGS_DIR`.
`FM_HOME` and `FM_ROOT_OVERRIDE` are the two that change how a case must be written: a case can no longer rely on an inherited `FM_HOME` and must name the home it means, and a helper that needs a repo root re-seeds `FM_ROOT_OVERRIDE` itself after sourcing the library, as `tests/wake-helpers.sh` does.
...
`config/firstmate-update-base` is inherited by secondmate homes since every home in a deployment updates from the same source.
...
FM_PROC_ROOT_OVERRIDE=   # alternate /proc root for the Linux process-identity reads - fm_pid_incarnation in fm-harness-pid-lib.sh and the image half fm_pid_identity adds in fm-wake-lib.sh - mainly for tests
...
FM_BACKEND_HERDR_NO_AUTOSTART=0  # herdr-only: 1 makes target_ready refuse to START a server, failing with "herdr server for session '<s>' is not running" instead; bin/fm-crew-state.sh exports it, spawn callers keep the start-then-poll path (docs/herdr-backend.md "Server start detachment")
...
FM_TAILSCALE_SOCKET=    # explicit tailscaled socket path the `tailscale` client dials, read ahead of the VESSEL_TAILNET_SOCKET declaration the vessel runtime exports; read for whether it is SET, so an empty value resolves nothing and hands the client back its own default, while leaving it unset falls through to that declaration (docs/lavish-access.md)
...
FM_SESSION_START_TIMING_KEEP=200   # session starts kept in state/session-start-timing.log, the per-step duration record bin/fm-session-start.sh appends one line to per run; at twice this many lines the log is rotated to session-start-timing.log.1, which is what keeps a line another session appended at the same moment rather than dropping it; the digest itself only names the total, and a run whose clock could not be read is recorded and reported as unreadable rather than as a 0ms run
FM_DECISION_LEDGER_MEMO_TTL=120   # seconds one full bin/fm-decision-ledger.sh read stays reusable by the next caller in the same session start, so bootstrap's audit and the digest's settled list do not walk the records twice; the reuse is keyed on the content of the record files and of the script itself, so an edit drops it whatever the age
FM_DECISION_LEDGER_NO_MEMO=0   # 1 recomputes the whole decision read on every call; it changes cost only, never a byte of output
...
FM_CHECK_TIMEOUT=30     # seconds allowed per slow check script; also bounds the merge-poll read bin/fm-teardown.sh makes before retiring a fulfilled poll on a refusal
...
FM_VALIDATION_DAEMON_TIMEOUT=5   # seconds bootstrap allows `no-mistakes daemon status` before reporting the reading as unestablished; a wedged daemon must not stall session start (docs/validation-daemon.md)
FM_VALIDATION_DAEMON_CHECK_DISABLE=0   # test-only: silence bootstrap's VALIDATION_DAEMON assertion, because every fixture's fake CLI invents its own daemon answer; tests/lib.sh sets it and tests/fm-validation-daemon-check.test.sh sets it back
FM_VALIDATION_DAEMON_FORCE_UNBOUNDED=0   # test-only: make that check behave as it would on a seat with neither timeout nor gtimeout, so the unbounded branch is exercised on a machine that has one
...
FM_DELIVERY_ENDPOINT_BACKEND_OVERRIDE=  # explicit backend for `fm-delivery-service.sh publish-endpoint`, for a session whose pane the discovery order cannot see; must be set together with the target override
FM_DELIVERY_ENDPOINT_TARGET_OVERRIDE=   # explicit pane target for that same publish; tmux targets are resolved to an exact pane and bound to the responding server's socket path and pid, while neither override still makes publish REFUSE rather than guess an address
...
FM_CLASSIFY_TERMINAL_CHECK_RE='^merged$'   # away-mode only: closed vocabulary of check outputs that cannot revert, so an identical repeat of one already escalated for the same pull request is self-handled; the first report and every non-terminal check still escalate
FM_STALE_ESCALATE_SECS=240         # idle seconds before an absorbed stale pane escalates, re-checked against the crew state at that moment: positive working evidence, or a run parked at a decision gate whose worker's agent process is confirmed alive, holds the ladder (bounded recheck once per FM_PAUSE_RESURFACE_SECS), while a crew with neither escalates; stale panes whose crew is not provably working surface immediately unless they declare the pause verb
FM_PAUSE_RESURFACE_SECS=3000       # seconds before an idle declared external wait re-surfaces for a recheck in the watcher or away-mode daemon; also the cadence of the bounded recheck for a wedge ladder held by positive working evidence (an active run or live-background-shells pane) or by a decision gate whose worker is confirmed alive. 50 minutes, deliberately under an hour: the seat prompt cache holds for one hour, so an hourly recheck lands just after the cache it would have reused expired. Parked rechecks that fall due together are carried on ONE wake record rather than one per task
FM_WEDGE_REPEAT_RESURFACE_SECS=3000 # maximum quiet window after one possible-wedge alarm for an unchanged pane/current-state class; defaults to FM_PAUSE_RESURFACE_SECS, every suppressed candidate remains in state/.wedge-alarm-history, and a state change re-arms immediate delivery; a continuously unchanged real wedge re-surfaces within this window plus FM_STALE_ESCALATE_SECS and FM_POLL
...
FM_BACKGROUND_WORK_REGEX='· [0-9]+ shells? ·'   # claude's live still-running-background-shells footer, read by fm-crew-state's pane fallback as working; the static previous-turn summary is excluded because it can outlive the footer count; never part of the busy set, so such a pane still goes stale and holds the ladder on the bounded recheck cadence
...
`fm-teardown.sh` retries Git's `Unable to create '...index.lock': File exists` return failure up to `FM_TREEHOUSE_RETURN_LOCK_RETRIES` times, and no other return failure is retried on the schedule these variables set.
The one other retried signature is unconfigured: a return refused as `is not managed by treehouse` is reissued exactly once under the pool's own spelling of the same directory, whose contract `bin/fm-teardown.sh`'s header owns and [`docs/slot-guard.md`](slot-guard.md) records.
