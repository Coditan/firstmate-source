...
FM_WAKE_BEAT_CONFIRM=90   # seconds the delivery stub gives a live, identity-matched watcher to emit one fresh beat before reporting its stale beacon; never consulted when no live watcher holds the lock, 0 disables it, and it must stay under FM_CODEX_WATCH_CHECKPOINT (docs/watcher-continuity.md "Surviving a host suspend")
