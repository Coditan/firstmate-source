...
A fork side that resolves to the same repository as the upstream side is refused as `FORK_SYNC_STUCK:` rather than compared, since comparing a repository with itself reports everything absorbed, which is the quietest possible wrong answer.
