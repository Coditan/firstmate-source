...
FM_SESSION_START_TIMING_KEEP=200   # session starts kept in state/session-start-timing.log, the per-step duration record bin/fm-session-start.sh appends one line to per run; the digest itself only names the total
