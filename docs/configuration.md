...
Editing `.gitignore` therefore selects every suite that asserts against the tracked ignore rules under `bin/fm-test-run.sh --changed`, not only the two ignore suites: `tests/fm-model-panel.test.sh` and `tests/fm-role-config.test.sh` assert that their own local config files are ignored, and a grep for a literal ignore line is what turned this change's first validation run red.
The `.gitignore` arm of the changed-file map in `bin/fm-test-run.sh` is the single owner of that list, so a new suite that asserts against `.gitignore` belongs in it; `tests/fm-test-run.test.sh` fails if one is missing.
