...
For Firstmate's pinned Herdr install, `bin/fm-install-herdr.sh` verifies Herdr `v0.7.4`.
The licence evidence measured for that pinned release is AGPL-3.0-or-later / commercial; do not generalize this statement to current upstream `master`, whose licence text differed when measured on 2026-08-17.
See the Herdr `v0.7.4` `LICENSE` file at `github.com/ogulcancelik/herdr`, or https://herdr.dev for Herdr's own published terms.
...
| Headless server start | `( ( trap '' HUP; HERDR_SESSION=<name> exec setsid herdr server --session <name> ) </dev/null >/dev/null 2>&1 & )`, dropping `setsid` where the host has none | A bare socket call does NOT auto-start the server; the adapter always starts-then-polls before any workspace/tab/pane call. This fact is for start only, not cleanup, and the explicit `--session` flag is intentional because `HERDR_SESSION` alone is not safe session targeting. The start execs the binary directly and fully detached; see "Server start detachment" below for why backgrounding a shell wrapper instead is a defect. |
...
- The start execs the herdr binary directly in the forked child, under `setsid` with stdin, stdout and stderr on `/dev/null`.
  ...
  macOS ships no `setsid(1)`, and much of this backend was verified on macOS, so `fm_backend_herdr_server_start_detached` checks `command -v setsid` and, when it is missing, detaches without it: the forked child ignores HUP (`trap '' HUP`, which survives `exec`), puts all three standard descriptors on `/dev/null` and execs the binary directly.
  The load-bearing property holds on every host: the server is an exec'd binary, never a backgrounded shell function, and holds none of the caller's descriptors, so no bash waits on it and the caller's pipe still reaches EOF.
  What the fallback gives up is only the separate process session, because without `setsid` there is no portable shell way to call `setsid(2)`; the server stays in the reader's process group, so a signal aimed at that group can still reach it, and the ignored HUP covers the common case of the reader's terminal going away.
  `tests/fm-backend-herdr.test.sh` covers both branches, forcing the fallback with a PATH that lacks `setsid`; the own-session assertion is made on the `setsid` branch only.
...
That verification did not cover a crew blocked on its OWN long-running foreground tool call - e.g. `no-mistakes axi run` without `--yes`, which blocks synchronously for the whole pipeline (minutes to tens of minutes) until a gate or outcome, per the `crew-dispatch` skill.
...
The target-existence primitive distinguishes a confirmed `pane_not_found` response from an unreadable server or malformed response, returning confidently absent only for the former.
...
Each adapter still owns its own capture and structural row-finding (genuinely different primitives), then hands the border-stripped candidate content plus a `<bordered>` flag to the shared classifier.
What counts as blank padding on that content is the shared owner's decision, not each adapter's: `fm_composer_classify_content` re-trims through `fm_composer_trim`, which strips ASCII/locale whitespace AND the U+00A0 no-break space claude pads its empty composer with, so an adapter whose own trim is ASCII-only still reaches the right verdict (see [claude's empty composer is padded with U+00A0](tmux-backend.md#claudes-empty-composer-is-padded-with-u00a0-verified-2026-08-10-claude-code-21226-tmux-34-on-linux) for the measurement and the two limits of it).
