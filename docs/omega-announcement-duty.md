...
For a secondmate seat what survives is a duty on the granting vessel, resting on measurement rather than argument: the incident recorded at `docs/herdr-backend.md:387` for 2026-07-13 reads "A routed request reached a Pi/Herdr secondmate without the visible `[fm-from-firstmate]` label, so the secondmate correctly treated it as direct captain conversation and returned nothing to the parent status path", a reproduced failure rather than a hypothetical, and it disproves the premise that such a seat simply takes typed text as its supervisor's ordinary instruction.
So the skill binds the sender rather than the seat: it sends omega text for a secondmate through the routed path that carries the from-firstmate marker, because with that carrier the seat reads the text as its supervisor's routed instruction and without it as the captain's own words. Nothing in this document is delivered to that seat.
