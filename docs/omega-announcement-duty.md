...
For a secondmate seat a receiving-seat bound survives, on measurement rather than argument: the incident recorded at `docs/herdr-backend.md:387` for 2026-07-13 reads "A routed request reached a Pi/Herdr secondmate without the visible `[fm-from-firstmate]` label, so the secondmate correctly treated it as direct captain conversation and returned nothing to the parent status path", a reproduced failure rather than a hypothetical, and it disproves the premise that such a seat simply takes typed text as its supervisor's ordinary instruction.
That is why the skill requires the from-firstmate carrier on text typed into a secondmate seat: with it the seat reads the announcement as its supervisor's routed instruction, and without it as the captain's own words.
