...
For a secondmate seat the premise that such a seat simply takes text typed into it as the ordinary instruction of its own supervisor does not hold, and the ground for saying so is measurement rather than argument: the incident recorded at `docs/herdr-backend.md:482` for 2026-07-13 reads "A routed request reached a Pi/Herdr secondmate without the visible `[fm-from-firstmate]` label, so the secondmate correctly treated it as direct captain conversation and returned nothing to the parent status path", a reproduced failure rather than a hypothetical.
