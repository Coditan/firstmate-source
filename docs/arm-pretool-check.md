...
It runs only in a primary home, and it denies only an executed `bin/fm-*.sh` command outside the recovery set that [`watcher-continuity.md`](watcher-continuity.md) lists, when task metadata is in flight and no identity-matched live watcher holds that home's lock.
Ordinary shell commands, fleet-script names used as data, all commands in an idle fleet, child worktrees, and the recovery commands themselves remain allowed.
The denial gives Claude reason-specific recovery guidance per the contract in [`watcher-continuity.md`](watcher-continuity.md): the session operating the home is told to drain, repair supervision through the two service scripts, and use fail-closed `bin/fm-teardown.sh` for completed tasks, while a task worker running the same hook from its worktree is told to report the stalled supervision and is handed no reserved command; that document owns who the refusal addresses and the limits of that split.
