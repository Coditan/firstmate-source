...
**`bin/fm-teardown.sh`** asks the question before cleanup and again at `teardown_treehouse_return`, using the owning home's state directory for nested secondmate cleanup, and performs hook and branch cleanup only after the destructive return or removal succeeds so a late ownership refusal leaves the worktree untouched.
