...
`bin/fm-tg-recv-arm.sh` keeps one home-scoped arm through `state/.tg-recv-arm.lock` and one receiver through `state/.tg-recv.lock`, while `docs/configuration.md` owns the local receiver split between the legacy captain line and the optional non-captain correspondent lane.
