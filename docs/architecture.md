...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, confirmed unhealthy certsync health or cannot-run certsync status reads from the heartbeat path, stale panes whose crew is not provably working and whose captain-relevant status the signal path has not already surfaced, whether their status log looks terminal or non-terminal, absorbed stale panes that persist past `FM_STALE_ESCALATE_SECS` and at that moment neither read as provably working nor sit at a decision gate a confirmed-alive worker can still answer, declared external waits (a crew's `paused:` status or a firstmate-declared `.parked-<window-key>` marker) that remain past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
Repeated stale escalations on the same unchanged pane and authoritative current-state class add an escalation count to the finding and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker.
The first finding is delivered immediately, while unchanged repeats are retained in the append-only `state/.wedge-alarm-history` and absorbed until `FM_WEDGE_REPEAT_RESURFACE_SECS` elapses; a changed pane or current-state class re-arms immediate delivery, and a failed history append fails open to delivery.
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: an actively running no-mistakes step for that crew's branch, a backend busy signature, or the harness's own indicator that its background shells are still running while its composer idles.
Codex has one stale-path-only addition: a non-terminal static pane that has no current-state proof and no busy row is still absorbed when `fm_backend_agent_alive` confidently reports the codex process alive, because codex-cli 0.145.0 can drop its busy row while a healthy turn is streaming or waiting in a tool call.
...
Parked tasks are marked in batches, so their rechecks come due together; one watcher pass gathers every parked window that is due and carries them all on a single wake record rather than one record per task, advancing each task's own throttle together (`parked_recheck_enqueue` in `bin/fm-watch.sh`).
That record lists the window names once and states one shared parked age for the whole set - a range when the ages differ - so a fleet-sized batch stays inside the drain's per-row echo bound instead of being shortened on the way to the seat.
The per-task cadence is unchanged by that: a due recheck left out of one gather stays due and is carried by the next pass.
A mid-pipeline run that `bin/fm-crew-state.sh` reports stopped at a no-mistakes decision gate is a distinct case from that operator-declared marker, and the watcher recognizes it without one: a worker parked at an ask-user finding is idle precisely because it followed its instructions, so once its stale pane has been surfaced it holds the wedge ladder on the bounded-recheck cadence instead of escalating.
The first sighting of that pane is surfaced like any other stopped crew, because a gate the crew never wrote a captain-relevant status for has reached nobody yet, and an absorb there would leave the decision waiting silently.
That is true for every harness including codex: an authoritative run-step gate is not the no-current-state-proof case the codex static-pane backstop covers, so it never reaches that backstop and codex gets no first-sight absorb the other harnesses lack.
...
That covers a required tool missing from the running service's `PATH`, a bounded run-step call that could not be made because no `timeout`, `gtimeout` or `perl` is reachable, and a call that ran and returned nothing at all.
Both answers that are not readings also carry an enumerated `cause:` token between the source and the prose detail, so a caller can switch on why there is no state instead of matching a sentence; `bin/fm-crew-state.sh`'s header owns that vocabulary and records which causes deliberately remain collapsed.
...
The harness block owns nothing of delivery any more: it states how a wake arrives in that seat's composer and what the seat does with it, and every seat's answer is the same drain-first step.
`bin/fm-delivery-service.sh` is the delivery half's service owner, the companion of `bin/fm-watcher-service.sh`: it converges or restarts only this home's listener unit, and it is where the locked session publishes the address its own model turn lives at (docs/wake-delivery.md).
...
Every due check in that sweep runs: a check that speaks queues its wake and the sweep continues, so no watch can starve the watches sorted behind it, and [seat-absence.md](seat-absence.md) records the 5.3-hour starvation that measured.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work is in flight or a durable queued wake is still unread, and either the watcher is unhealthy or this home's delivery listener is down (a live identity-matched away daemon standing in for it while `state/.afk` is present), direct Stop hooks block and passive turn-end hooks force one bounded follow-up.
...
A captain-relevant status the signal path already surfaced does not wake firstmate a second time when the same crew's pane then settles stale; the suppression is keyed on the surfaced status content, never on the window, and it still starts the wedge timer.
...
Independently, `fm-spawn.sh`, `fm-send.sh`, `fm-teardown.sh`, and `fm-model-panel.sh` source `bin/fm-gate-refuse-lib.sh` and exit with status 3 before fleet mutation when the gate environment marker is present or the current checkout matches the default no-mistakes gate-repository topology.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before calling the provider's supported merge client.
A pull request that no task in this home owns, because its task was torn down after the work finished or because another vessel built it and handed it over for this vessel's merge decision, is landed by the same helper's `--no-local-task` form, which carries no task id, records nothing, and says so in its own output rather than succeeding silently; the helper's header owns why a merge with no local cleanup to protect is not a weakening of the recording requirement.
The helper requires a canonical full pull request URL, supports GitHub and the home's configured Forgejo instance, defaults to a real merge commit, translates supported merge-method flags for the selected client, and rejects malformed URLs or repository override flags before recording merge state; a well-formed GitLab merge request URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)) is refused too, explicitly, rather than sent to the wrong forge.
The Forgejo path requires the expected head commit and reports that its client provides neither branch deletion nor a merge watch; [docs/forgejo-merge-helper.md](forgejo-merge-helper.md) owns the detailed contract and verification boundary.
...
On GitHub it also has the forge delete the merged head branch as part of that merge; Forgejo provides no equivalent through its client, and [docs/merged-branch-cleanup.md](merged-branch-cleanup.md) owns that distinction, the other producers, and the pipeline-mirror gap deliberately left unbuilt.
...
Home-domain captain preferences go to `data/captain.md`, cross-domain shared captain preferences go to the primary home's `data/captain-shared.md`, fleet-local operational knowledge is split across the loaded and on-demand files with [`.agents/skills/stow/SKILL.md`](../.agents/skills/stow/SKILL.md) owning which one a new learning goes to, project-intrinsic knowledge goes through normal crewmate delivery into that project's committed `AGENTS.md`, and task-scoped notes or undone next steps go to the backlog.
...
Every guarded Bridge envelope relay call refreshes the Bridge clone through that same single sanctioned path before dispatching, and a read-shaped call refuses loudly instead of answering from a clone the refresh did not prove current; `bin/fm-bridge-relay.sh`'s header owns that contract.
...
`bin/fm-firstmate-update-check.sh` reports source-only instruction-surface changes without updating the checkout.
It is run by the daily currency round rather than by an external timer, because on the vessel that first needed it no timer had ever been installed and the check had therefore never produced a reading; [currency-round.md](currency-round.md) records that measurement and the mechanism it produced.
That check does not cover drift between the primary checkout's own default branch and its own origin, so bootstrap's `self_drift_check` fills that gap: on the primary's default branch it fetches origin under a bounded `FM_SELF_DRIFT_BOOTSTRAP_TIMEOUT` (default 10s), then compares local and `origin/<default>` with `rev-list`/`merge-base`, and is stateless and detect-only - it never fast-forwards, checks out, merges, or stashes.
...
Those knowledge files only stay affordable if something re-measures them, so `bin/fm-nudge.sh` runs on the same watcher seam and asks each vessel to measure and curate its own pair every 48 hours, and to sweep its own repositories' design every 52; [nudge-cadence.md](nudge-cadence.md) records what the files had grown to before anything did, and why both duties ride one check rather than a unit each.
`bin/fm-forge-status.sh` rides that same seam to read the forge's own status page, appending each new reading to a durable log and waking firstmate only when the reading changes, so an outage reaches the fleet as a recorded fact rather than through workers debugging their own innocent code; [forge-status-watch.md](forge-status-watch.md) records the outage that commissioned it and why the judgement stays with firstmate.
