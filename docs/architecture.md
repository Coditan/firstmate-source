...
Those knowledge files only stay affordable if something re-measures them, so `bin/fm-curation-nudge.sh` runs on the same watcher seam every 48 hours and asks each vessel to measure and curate its own pair; [curation-nudge.md](curation-nudge.md) records what they had grown to before anything did.
