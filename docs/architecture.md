...
The curator-only `bin/fm-fork-sync-check.sh` reports when the curated fork still lacks real-upstream content - rebased or squashed upstream commits already present by equivalent patch or identical touched files do not count - and points its patch review at `docs/fork-patches.md`.
