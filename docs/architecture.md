...
The watcher also measures the primary session's own context against the captain's ceiling and, when the session is over it at a quiet boundary, either queues one wake that carries the whole stow-then-clear procedure or, because a reset during a live conversation wipes the scrollback and can answer a queued message into a context that is then discarded, asks the captain first.
Separately, and regardless of size or quiet, it reports the ceiling as unenforced whenever a running session cannot be measured at all, and reports a blocked reset when the re-entry path is broken, because a ceiling nobody can measure is one nobody is holding.
