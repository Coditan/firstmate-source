...
## A generated artifact must be scoped to its own repository

A file that is generated from the tree, committed, and then checked by a vendored test cannot be correct in both repositories unless the generator is scoped to the repository it describes.

`docs/command-domain-map.md` is generated by `bin/fm-toolbelt-domain-map.py`, and both it and `tests/fm-toolbelt-domain-map.test.sh` are vendored.
Measured 2026-09-08 on a composed tree, the unscoped generator scanned `git ls-files` across the whole root, so the fleet's own manifest and its `fleet/` namespace became references: 188 of the map's rows gained a reference to a path this repository does not carry, the vendored freshness check failed, and there was no repair on the vendoring side, because `.fleet-overlay` cannot override a vendored file and `.fleet-excluded` would delete it.

The generator now declares the paths this repository owns and scans only those, so it produces the same map in either tree.
The workflows are declared one file at a time rather than as a directory, because `.github/workflows/` is the one directory both repositories write to, and a fleet workflow there would otherwise be read as this repository's own.
The rule generalizes to any later generated artifact: scope it to its own repository's namespace, declared positively so no other repository's layout is encoded in this one, and report what the scan skipped rather than dropping it silently.
