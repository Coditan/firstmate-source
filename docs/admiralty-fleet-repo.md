...
- Restoring the ancestry as this fork's `main` advances past `e52cc76` is an open captain decision, not a solved problem.
  ...
  **The procedure has since been applied.**
  ...
  This fork's `main` has since advanced past the pin again (see the top of this section), so the same open question - restoring ancestry as the fork keeps moving - recurs at every pin bump and each one needs its own merge, not just the first.
