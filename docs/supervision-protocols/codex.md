Mode: external wake delivery, Codex primary.

This session holds no wake-delivery object of any kind.
The foreground checkpoint this seat used to run is gone: the listener that turns a queued wake into a turn is a service outside this harness, supervised the way the watcher loop is.
...
2. Do not arm anything afterwards: no checkpoint, no bounded foreground wait, no other blocking delivery call.
   Nothing blocks reasoning to keep supervision alive any more.
