Mode: external wake delivery, Pi primary.

This session holds no wake-delivery object of any kind.
The Pi watch extension that used to own the delivery child is gone: the listener that turns a queued wake into a turn is a service outside this harness, supervised the way the watcher loop is.
...
2. Do not arm anything afterwards.
   The Pi arm tool and its slash command are both gone.
...
5. The turn-end guard extension at `__FM_PI_TURNEND_EXT__` is still required and still auto-loads once the project is trusted; restart with `-e __FM_PI_TURNEND_EXT__` if it is not loaded.
