...
Session start then asks `bin/fm-sessionstart-nudge.sh --rebind-to-lock` to rebind this home's context-ceiling transcript record to the holder the new lock names, as it does after every acquisition, because the SessionStart hook ran before the supersede and correctly wrote nothing for a foreign record.
A redeemed handover ticket is the one acquisition that does not run through session start, so `fm-lock.sh` invokes that same rebind itself after publishing the successor and prints what it did with the acquisition line; [sessionstart-nudge.md](sessionstart-nudge.md) owns that record and what the rebind can and cannot establish.
