...
Once the bound is spent the episode is over for that condition and the respawner returns at the bound test on every later cycle while its process keeps beating, so `status` answers `gave-up:` while `state/.seat-respawn-giveup` stands; the episode itself is cleared only by a changed delivery status, a seat taking this home's lock, or the stay-down marker.
A give-up on record always refers to the episode currently being counted, because the respawner drops it whenever it writes `state/.seat-respawn-attempts` under a different condition key - a dead seat's blocked reason is not stable, and without that a give-up left by one episode would be read as belonging to a later one launching under the same reason.
