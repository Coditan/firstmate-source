...
While the record stands the launch is withheld, but the wait is not free: each cycle that is otherwise due still spends a launch attempt, so a first turn that never lands reaches the retry bound, gives up, and emits its give-up finding exactly as an exhausted relaunch episode does.
