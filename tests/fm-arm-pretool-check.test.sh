...
matrix_case A01 allow 'bin/fm-tg-recv-arm.sh'
matrix_case A02 allow './bin/fm-tg-recv-arm.sh --restart'
matrix_case A03 allow 'exec bin/fm-tg-recv-arm.sh'
matrix_case A04 allow 'bin/fm-tg-recv-arm.sh --attach'
matrix_case A05 allow 'exec bin/fm-tg-recv-arm.sh --attach'
matrix_case A06 allow "$ROOT/bin/fm-tg-recv-arm.sh --attach"
matrix_case A07 allow "cd '$ROOT'; exec bin/fm-tg-recv-arm.sh"
matrix_case A08 allow "cd '../firstmate'; bin/fm-tg-recv-arm.sh --attach"
matrix_case A09 allow "export FM_HOME='$ROOT'; bin/fm-tg-recv-arm.sh --attach"
matrix_case A10 allow 'source config/x-mode.env; bin/fm-tg-recv-arm.sh --attach'
matrix_case A11 allow "source 'config/x-mode.env'; bin/fm-tg-recv-arm.sh --attach"
matrix_case A12 allow "source './config/x-mode.env'; bin/fm-tg-recv-arm.sh --attach"
matrix_case A13 allow "source '$ROOT/config/x-mode.env'; bin/fm-tg-recv-arm.sh --attach"
matrix_case A14 allow "[ -f 'config/x-mode.env' ] && source 'config/x-mode.env'; exec bin/fm-tg-recv-arm.sh"
matrix_case A15 allow "cd $ROOT && exec bin/fm-tg-recv-arm.sh"
matrix_case A16 allow "export FM_HOME=$ROOT && bin/fm-tg-recv-arm.sh --attach"
matrix_case A17 allow $'source "config/x-mode.env"\nbin/fm-tg-recv-arm.sh --attach'
...
matrix_case R03 allow "rg -n 'fm-tg-recv-arm.sh &' docs tests"
matrix_case R04 allow "rg -n 'bin/fm-tg-recv-arm.sh; echo bad' docs"
matrix_case R05 allow "git grep 'fm-tg-recv-arm.sh && echo bad'"
matrix_case R06 allow "sed -n '/fm-tg-recv-arm.sh/p' docs/arm-pretool-check.md"
matrix_case R07 allow 'assert_contains "$content" '\''fm-tg-recv-arm.sh &'\'''
matrix_case R08 allow "printf '%s\\n' 'bin/fm-tg-recv-arm.sh --attach >/tmp/out'"
matrix_case R09 allow "tmux send-keys -t isolated-pi-lab 'bin/fm-tg-recv-arm.sh &' Enter"
matrix_case R10 allow "tmux send-keys -t isolated-pi-lab \"printf '%s\\n' 'bin/fm-tg-recv-arm.sh &'\"; tmux send-keys -t isolated-pi-lab Enter"
matrix_case R11 allow "python3 -c 'print(\"bin/fm-tg-recv-arm.sh; echo data\")'"
matrix_case R12 allow "bash -lc \"rg -n 'fm-tg-recv-arm.sh &' docs\""
matrix_case R13 allow "echo 'pkill -f fm-watch'"
matrix_case R14 allow "rg -n 'pkill -f fm-watch' docs tests"
matrix_case R15 allow "echo ok # bin/fm-tg-recv-arm.sh &"
matrix_case R16 allow $'# bin/fm-tg-recv-arm.sh &\necho ok'
matrix_case R17 allow "printf '%s\\n' 'fm-watch.sh; a && b || c > out' | sed -n '1p'"
matrix_case R18 allow "sh -c 'tmux send-keys -t lab \"bin/fm-tg-recv-arm.sh &\" Enter'"
matrix_case R19 allow "eval 'printf \"%s\\n\" \"bin/fm-tg-recv-arm.sh &\"'"
matrix_case R20 allow "rg -n 'fm-tg-recv-arm.sh &' docs tests"

matrix_case D01 deny 'bin/fm-tg-recv-arm.sh &'
matrix_case D02 deny 'nohup bin/fm-tg-recv-arm.sh'
matrix_case D03 deny 'bin/fm-tg-recv-arm.sh & disown'
matrix_case D04 deny '(bin/fm-tg-recv-arm.sh) &'
matrix_case D05 deny "bash -lc 'bin/fm-tg-recv-arm.sh &'"
matrix_case D06 deny '$(bin/fm-tg-recv-arm.sh)'
matrix_case D07 deny 'echo "$(bin/fm-tg-recv-arm.sh --attach)"'
matrix_case D08 deny 'cat <(bin/fm-tg-recv-arm.sh)'
matrix_case D09 deny 'bin/fm-tg-recv-arm.sh >/tmp/out'
matrix_case D10 deny 'bin/fm-tg-recv-arm.sh --attach </dev/null'
matrix_case D11 deny 'bin/fm-tg-recv-arm.sh 2>&1 | head -2'
matrix_case D12 deny 'bin/fm-tg-recv-arm.sh | cat'
matrix_case D13 deny 'bin/fm-tg-recv-arm.sh --attach | timeout 1 cat'
matrix_case D14 deny 'echo before; bin/fm-tg-recv-arm.sh'
matrix_case D15 deny 'bin/fm-tg-recv-arm.sh --attach; echo after'
matrix_case D16 deny 'true && bin/fm-tg-recv-arm.sh'
matrix_case D17 deny 'bin/fm-tg-recv-arm.sh --attach || true'
matrix_case D18 deny $'bin/fm-tg-recv-arm.sh\nbin/fm-tg-recv-arm.sh --attach'
...
matrix_case D24 deny $'bin/fm-tg-rec\\\nv-arm.sh &'
matrix_case D25 deny 'sudo -u root bin/fm-tg-recv-arm.sh &'
matrix_case D26 deny 'env -u PATH bin/fm-tg-recv-arm.sh &'
matrix_case D27 deny "bash -c \$'bin/fm-tg-recv-arm.sh &'"
matrix_case D28 deny $'bash <<\'EOF\'\nbin/fm-tg-recv-arm.sh &\nEOF'
matrix_case D29 deny "WATCHER='bin/fm-tg-recv-arm.sh &' bash -c 'eval \"\$WATCHER\"'"
matrix_case D30 deny "p=\$(pgrep -f '/bin/fm-watch.sh'); kill \"\$p\""
matrix_case D31 deny "env -S 'bin/fm-tg-recv-arm.sh &'"
matrix_case D32 deny "env --split-string='$ROOT/bin/fm-tg-recv-arm.sh &'"
matrix_case D33 deny 'bin/fm-"tg-recv-arm.sh" &'
matrix_case D34 deny "WATCHER='bin/fm-tg-recv-arm.sh'; \"\$WATCHER\" &"
matrix_case D35 deny "bash -c -- 'bin/fm-tg-recv-arm.sh &'"
matrix_case D36 deny 'bash bin/fm-tg-recv-arm.sh &'
matrix_case D37 deny '. bin/fm-tg-recv-arm.sh &'
matrix_case D38 deny "bash <<< 'bin/fm-tg-recv-arm.sh &'"
matrix_case D39 deny "eval 'true;' 'bin/fm-tg-recv-arm.sh &'"
matrix_case D40 deny 'timeout 30 bin/fm-tg-recv-arm.sh &'
matrix_case D41 deny 'gtimeout 30 bin/fm-tg-recv-arm.sh &'
matrix_case D42 deny 'bin/fm-tg-recv-arm{,-alt}.sh &'
matrix_case D43 deny 'bin/fm-tg-recv-arm.sh* &'
matrix_case D44 deny "pattern='fm-watch'; pkill -f \"\$pattern\""
matrix_case D45 deny "p=\$(pgrep -f '/bin/fm-watch.sh'); q=\$p; kill \$q"
matrix_case D46 deny '$FM_HOME/bin/fm-tg-recv-arm.sh &'
matrix_case D47 deny '$HOME/firstmate/bin/fm-tg-recv-arm.sh | cat'
matrix_case D48 deny '~/firstmate/bin/fm-tg-recv-arm.sh &'
...
matrix_case D52 deny "bin/fm-\$'\x74'g-recv-arm.sh &"
matrix_case D53 deny 'bin/fm-$"tg"-recv-arm.sh &'
matrix_case D54 deny 'bin/fm-tg-recv-$"arm".sh &'
...
matrix_case E01 allow "bin/fm-tg-recv-arm.sh --seconds '180;still-one-arg'"
matrix_case E02 allow "bin/fm-tg-recv-arm.sh --label 'fm-tg-recv-arm.sh; literal argument'"
matrix_case E03 allow 'bin/fm-tg-recv-arm.sh # output > file &'
matrix_case E04 allow $'# setup comment with fm-watch.sh; && >\nsource "config/x-mode.env"\nbin/fm-tg-recv-arm.sh --attach'
matrix_case E05 deny "FM_HOME=$ROOT bin/fm-tg-recv-arm.sh --attach"
matrix_case E06 deny "env FM_HOME=$ROOT bin/fm-tg-recv-arm.sh"
matrix_case E07 deny "source '/tmp/not-firstmate/config/x-mode.env'; bin/fm-tg-recv-arm.sh --attach"
matrix_case E08 deny "bash -lc 'bin/fm-tg-recv-arm.sh --attach'"
matrix_case E09 deny '(bin/fm-tg-recv-arm.sh --attach)'
matrix_case E10 deny "eval 'bin/fm-tg-recv-arm.sh &'"
matrix_case E11 deny "exec bash -lc 'bin/fm-tg-recv-arm.sh &'"
matrix_case E12 allow 'bash -lc "$WATCHER_COMMAND" # fm-tg-recv-arm.sh'
matrix_case E13 allow "printf '%s\\n' 'argument has ; and fm-tg-recv-arm.sh and &&'"
matrix_case E14 allow '$FM_HOME/bin/fm-teardown.sh &'
matrix_case E15 allow '$FM_HOME/bin/fm-tg-recv-arm.sh'
matrix_case E16 allow '~/firstmate/bin/fm-tg-recv-arm.sh --attach'
...
test_direct_policy_contract() {
  ...
  assert_policy direct-pipeline $'deny\twatcher-pipeline' 'bin/fm-tg-recv-arm.sh | cat'
  assert_policy direct-leading-redirection $'deny\twatcher-redirection' '>/tmp/out bin/fm-tg-recv-arm.sh'
  assert_policy direct-unclassifiable $'deny\tunclassifiable-protected-command' "bin/fm-tg-recv-arm.sh 'unterminated"
  assert_policy direct-unsupported $'deny\tunclassifiable-protected-command' 'if true; then bin/fm-tg-recv-arm.sh; fi'
  assert_policy direct-constructed-payload $'deny\twatcher-nested' "WATCHER='bin/fm-tg-recv-arm.sh &'; bash -lc \"\$WATCHER\""
  assert_policy direct-parameter-export allow 'export FM_HOME=${HOME}; bin/fm-tg-recv-arm.sh --attach'
  assert_policy direct-expanded-arm-blessed allow '$FM_HOME/bin/fm-tg-recv-arm.sh'
  assert_policy direct-expanded-arm-background $'deny\twatcher-background' '$FM_HOME/bin/fm-tg-recv-arm.sh &'
  assert_policy direct-expanded-arm-pipeline $'deny\twatcher-pipeline' '$HOME/firstmate/bin/fm-tg-recv-arm.sh | cat'
  ...
  heredoc_watcher=$'bin/fm-tg-recv-arm.sh <<\'EOF\'\ndata only\nEOF'
...
test_command_equals_form() {
  "$CHECK" --command='bin/fm-tg-recv-arm.sh &' >/dev/null 2>&1
...
test_background_flag_accepted_and_non_gating() {
  local rc_bg rc_nobg
  "$CHECK" --command 'exec bin/fm-tg-recv-arm.sh' --background true >/dev/null 2>&1
  rc_bg=$?
  "$CHECK" --command 'exec bin/fm-tg-recv-arm.sh' >/dev/null 2>&1
  rc_nobg=$?
...
test_stdin_grok_schema_deny() {
  local out rc
  out=$(printf '%s' '{"toolInput":{"command":"bin/fm-tg-recv-arm.sh &","background":false},"toolName":"run_terminal_command"}' | "$CHECK" 2>/dev/null)
  rc=$?
...
test_stdin_claude_codex_schema_allow() {
  local rc
  printf '%s' '{"tool_input":{"command":"exec bin/fm-tg-recv-arm.sh"},"tool_name":"Bash"}' | "$CHECK" >/dev/null 2>&1
  rc=$?
...
test_stdin_claude_codex_schema_deny() {
  local rc
  printf '%s' '{"tool_input":{"command":"bin/fm-tg-recv-arm.sh &"},"tool_name":"Bash"}' | "$CHECK" >/dev/null 2>&1
  rc=$?
...
test_prefilter_is_strict_superset() {
  ...
  "$CHECK" --command "$(printf 'bin/fm-tg-rec\\\nv-arm.sh &')" >/dev/null 2>&1
  rc=$?
  ...
  "$CHECK" --command "bin/fm-\$'\x74'g-recv-arm.sh &" >/dev/null 2>&1
  rc=$?
...
test_failopen_missing_jq() {
  ...
  PATH="$fakebin" bash -c "printf '%s' '{\"tool_input\":{\"command\":\"bin/fm-tg-recv-arm.sh &\"}}' | '$CHECK'" >/dev/null 2>&1
  rc=$?
...
test_failopen_missing_node() {
  ...
  PATH="$fakebin" "$CHECK" --command 'bin/fm-tg-recv-arm.sh &' >/dev/null 2>&1
  rc=$?
...
test_claude_mode_stdout_empty_on_deny() {
  ...
  out=$("$CHECK" --claude --command 'bin/fm-tg-recv-arm.sh &' 2>"$stderr_file")
  rc=$?
...
test_default_mode_stdout_has_grok_json_on_deny() {
  local out rc
  out=$("$CHECK" --command 'bin/fm-tg-recv-arm.sh &' 2>/dev/null)
  rc=$?
...
test_allow_is_silent_both_modes() {
  local out1 out2
  out1=$("$CHECK" --command 'exec bin/fm-tg-recv-arm.sh' 2>&1)
  out2=$("$CHECK" --claude --command 'exec bin/fm-tg-recv-arm.sh' 2>&1)
