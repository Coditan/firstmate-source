...
# Section 9 tells firstmate to translate its own vocabulary away, and
# domain-modeling tells it never to translate a proper noun. Read alone, each
...
test_ahoy_user_role_injections_share_one_marker() {
  ...
  assert_contains "$spawn" 'launch-pointer __BRIEF__' \
    "cross-harness launches do not use the canonical launch-instruction constructor"
