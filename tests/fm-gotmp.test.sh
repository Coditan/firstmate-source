...
make_fake_root() {
  ...
  ln -s "$ROOT/bin/fm-axi-path-lib.sh" "$fake/bin/fm-axi-path-lib.sh"
  ...
  # fm-landing-remote-lib.sh: teardown sources the shared remote filter before
  # deciding whether a worktree has preserved commits.
  ln -s "$ROOT/bin/fm-landing-remote-lib.sh" "$fake/bin/fm-landing-remote-lib.sh"
