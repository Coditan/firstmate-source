...
test_teardown_conformance_old_vs_new() {
  ...
  assert_contains "$(cat "$log_new")" "treehouse"$'\x1f''status' \
    "teardown did not inspect pool ownership before returning the worktree"
  ...
  pass "fm-teardown.sh: ownership inspection precedes the unchanged treehouse return + tmux kill-window operations"
}
