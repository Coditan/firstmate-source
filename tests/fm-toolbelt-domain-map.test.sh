...
test_outside_namespace_files_do_not_change_the_map() {
  local tmp repo before after control
  ...
  "$repo/bin/fm-toolbelt-domain-map.py" --root "$repo" --stdout >"$after"
  cmp -s "$before" "$after" \
  ...
  "$repo/bin/fm-toolbelt-domain-map.py" --root "$repo" --stdout >"$control"
...
test_check_reports_skipped_outside_entries() {
  local tmp repo out
  ...
  assert_contains "$out" "scan skipped 2 entries" "check did not report what it skipped"
  ...
  # of the two it is.
  assert_contains "$out" "namespace: .github/workflows/composing-ci.yml, outside" \
    "skip report did not name a whole undeclared directory and an exact file inside a shared one"
  assert_not_contains "$out" ".github/workflows/ci.yml" \
    "skip report named a declared file as skipped"
  pass "check mode reports the entries its scan skipped"
}
