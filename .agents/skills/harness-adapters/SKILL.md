...
This is separate from the per-task crewmate turn-end hook above (that one just `touch`es a marker file through the task's `.claude/settings.fm-task.json`, loaded explicitly with `--settings`).
