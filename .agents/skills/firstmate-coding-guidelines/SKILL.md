...
Skills installed from upstream and listed in `skills-lock.json` are exempt from the locally authored frontmatter requirements so their recorded content stays untouched, but each still needs exactly one section 13 entry.
...
A section 13 entry states its condition once.
Where an operating section already states that same condition at the point it fires, the entry cross-references that section instead of restating it, because two copies in one always-loaded file drift the way the bootstrap diagnostic-prefix list did: `AGENTS.md` had lost `WATCHER_UNIT` and `FREQUENCY_MONITOR_UNIT` while `bootstrap-diagnostics` still carried both.
Reducing an entry below its stated condition, to a bare skill name, is not available: the entry is one of only two arrival routes and the harness listing is the other, and `tests/fm-instruction-owners.test.sh` also pins the exact `AGENTS.md` wording of several entries.
