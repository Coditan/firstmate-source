...
**Care:** a tool announcing its own gap is not a tool being watched. `no-mistakes` printed "A new version of no-mistakes is available" on every invocation, to every agent, for weeks while twelve releases behind, and this sweep still found it in 2026-08-31 with owner "nobody" - the notice was never missing, an owner was. Judge by whether something re-measures the tool, never by whether the gap was visible (`docs/currency-round.md` "The tool that announced its own gap" owns that incident and the reading that closed it).
