...
- Firstmate may attach to one of this vessel's own seats over tmux and type directly into that seat's composer.
  ...
  The split falls where it does because the tool is required wherever the tool can carry it and the hand path survives only where no routed path exists: a seat stopped at a confirmation prompt is not at its composer but blocked waiting for a keystroke, which `bin/fm-send.sh` cannot reach, so composer typing stands for answering that prompt.
  There firstmate holds what the hand path does not reproduce - it carries the label only, neither the resting-to-active switch nor the pending-reply correlation is armed, and the seat's reply may therefore never reach routed status - so it must not read that silence as nothing to report.
