...
- Firstmate may attach to one of this vessel's own seats over tmux and type directly into that seat's composer.
  ...
  Naming secondmates there adds no reach, on evidence this repository cannot check for itself: a reading taken in the fleet repository on 2026-09-08 - `fleet/doctrine/peer-dispatch.md` does not live here, so nothing in this worktree confirms it - reports line 55 of the fleet's cross-vessel rule as "Not in scope: a vessel steering its own crewmates, its own scouts, or its own secondmates. Those are its own direct reports in its own home, and bin/fm-send.sh is the correct tool.", which puts a secondmate seat outside that ban, so it was never another vessel's seat for it.
  ...
  At a secondmate's seat the line falls between two acts - the grant permits looking at the composer's own state and at the content of a prompt firstmate is answering under the confirmation grant, which it cannot weigh against the bounds below without reading, and it does not permit reading that chat for state - so firstmate still takes that secondmate's state from its routed status and referenced documents (`AGENTS.md:229`) and still does not treat a quiet pane as stale (`AGENTS.md:331`).
  That routed state only exists if the request is delivered the way that arms it, and a carrier typed in by hand reproduces the label alone, so the named task's text for a secondmate seat goes through `bin/fm-send.sh` against a metadata-routed `kind=secondmate` selector.
  That is what the fleet rule quoted above already calls the correct tool, and it is the one path that arms all three behaviours together: the resting-to-active switch (`bin/fm-send.sh:238`), the pending-reply correlation id recorded before delivery (`bin/fm-send.sh:247-248`, `bin/fm-pending-reply-lib.sh:5-18`), and the marker.
  Unmarked text is read as direct captain conversation and returns nothing to the parent status path (`docs/herdr-backend.md:387`, a reproduced incident), so a grant that let firstmate type in a way that severs the reply route while the line above sends it to that route for state would be this file arguing with itself.
  The split falls where it does because the tool is required wherever the tool can carry it, and it carries more than composer text: a confirmation one of the backend's supported keys answers goes through `bin/fm-send.sh --key` (`bin/fm-send.sh:9-11`; the tmux path is a bare `tmux send-keys` after target resolution, `bin/backends/tmux.sh:70-73`).
  The tool is required there for a different reason than above, and the difference matters: `--key` arms nothing, being left unmarked and returning before the marking block (`bin/fm-send.sh:211`, `:229-233`), and nothing arms a reply route for a keystroke because a keystroke expects no reply - the tool is required because it resolves the target and refuses an unresolved guess instead of falling back to a window search (`bin/fm-send.sh:6-8`).
  Hand typing survives for the narrow residue no supported key can answer at all, and today for the announcement.
  The announcement rides the hand path because every marked text send opens a pending-reply record before delivery (`bin/fm-send.sh:236-249`) that only a status line carrying that record's own corr resolves (`bin/fm-pending-reply-lib.sh:460-500`), and an informational announcement earns no such report, so routing it would leave one uncorrelated record per seat per window; `bin/fm-send.sh` cannot send marked text that expects no reply, so this is today's route rather than the intended one.
  Hand-typed text still carries the marker: labelling the text for the receiving seat was always the label's job and the reply route is a separate mechanism, so the label goes on by hand even where the route does not.
  Neither unrouted act arms the guard - a `--key` answer and hand-typed text alike leave the resting-to-active switch and the pending-reply correlation unarmed - so nothing will notice a missing report for either.
  Firstmate therefore establishes the outcome of an answer it gave in that seat itself, reading that seat for what its own act did there and for nothing else, rather than waiting on a correlation that never armed; that is the composer's own state, which the line above already permits, and the secondmate's work state still comes only from its routed status and referenced documents.
  `bin/fm-operational-input.sh` owns the carrier's construction, parsing and marker bytes, with `bin/fm-marker-lib.sh` as the sourced compatibility path, and `tests/fm-send-secondmate-marker.test.sh:4-9` and `docs/architecture.md:197` pin the behaviour; none of its format or mechanics is restated here.
