...
- Firstmate may attach to one of this vessel's own seats over tmux and type directly into that seat's composer.
  ...
  That routed state only exists if the typing carries it, so text typed into a secondmate seat under this grant goes in with the same from-firstmate carrier `bin/fm-send.sh` prepends for a resolved `kind=secondmate` target.
  ...
  `bin/fm-send.sh` and `bin/fm-marker-lib.sh` own the carrier and `tests/fm-send-secondmate-marker.test.sh:4-9` and `docs/architecture.md:197` pin its behaviour; none of its format or mechanics is restated here.
