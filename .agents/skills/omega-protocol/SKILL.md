...
- Firstmate may attach to one of this vessel's own seats over tmux and type directly into that seat's composer.
  ...
  That routed state only exists if the request is delivered the way that arms it, and a carrier typed in by hand reproduces the label alone, so ordinary omega text for a secondmate seat - the announcement and the named task's text - goes through `bin/fm-send.sh` against a metadata-routed `kind=secondmate` selector.
  ...
  The split falls where it does because the tool is required wherever the tool can carry it, and it carries more than composer text: `bin/fm-send.sh` also answers a blocked seat with `--key`, for the keys its backend supports (`bin/fm-send.sh:9-11`; the tmux path is a bare `tmux send-keys` after target resolution, `bin/backends/tmux.sh:70-73`), so a confirmation a supported key answers goes through the tool like any other omega text.
  Hand typing survives only for the narrow residue no supported key can answer at all.
  It still carries the marker there: labelling the text for the receiving seat was always the label's job and the reply route is a separate mechanism, so the label goes on by hand even where the route does not.
  A hand-typed answer arms nothing else - not the resting-to-active switch, not the pending-reply correlation - so no routed reply is coming for it, and firstmate reads that seat for the outcome of that answer instead of waiting for one.
