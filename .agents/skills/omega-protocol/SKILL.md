...
What is permitted before the announcement is bounded by function and not by count: firstmate may answer whatever is currently stopping the composer from taking the announcement, however much of it there turns out to be.
It is written as one bound on purpose, because two bounds over the same act can return opposite verdicts and every state where they disagree is a state the rule cannot answer, while one bound cannot disagree with itself; a reader who adds a count back here reopens exactly that.
