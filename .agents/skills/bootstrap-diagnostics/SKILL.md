...
- `VALIDATION_DAEMON: the validation pipeline daemon is not running ...` - the daemon behind every review gate on this machine is down, so every parked run is unanswerable and no worker will discover it until one tries to answer its own gate and gets a refused socket back.
