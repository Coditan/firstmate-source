...
- `RESPAWNER_UNIT: systemd --user is unavailable and tmux is not installed ...` - neither supervision tier is available here, so nothing would restart a dead primary seat and it would need a human; report it rather than treating the absence of a seat as recoverable.
- `RESPAWNER_UNIT: systemd --user unavailable; the lock-holding session will start the tmux keeper fallback` - this is the automatic fallback tier and needs no captain consent; it is the detect-only reading, so a session holding the fleet lock starts the keeper itself.
  Do NOT read this as "a dead seat needs manual relaunch": the keeper tier restarts the seat, and it is re-ensured from the watcher rather than from a session, so it keeps working while the seat is the part that is gone (`docs/seat-absence.md`).
- `RESPAWNER_UNIT: systemd --user unavailable and the tmux keeper fallback failed ...` - the fallback tier was selected and could not be started, so nothing would bring the first mate back; inspect `bin/fm-seat-respawner-service.sh status`, this home's respawner lock and beacon, and `state/.seat-respawner.log`, then report the concrete failure.
- `RESPAWNER_UNIT: nothing keeps this vessel's first-mate restart running between sessions ...` - the watcher-hosted check that re-ensures the keeper tier is not armed, so the restart would stop with the session that started it; run `bin/fm-seat-respawner-service.sh --arm` and report the reason if it refuses.
- `RESPAWNER_UNIT: this vessel's first-mate restart was armed <n>s ago and has never run ...` or `... last ran <n>s ago and has stopped ...` - the restart is armed but the monitoring service is not running it, or has never once run it, which is a supervision fault rather than a seat one: run `bin/fm-seat-respawner-service.sh restart`, confirm with `bin/fm-seat-respawner-service.sh status`, and repair the watcher through the emitted supervision instructions if the beacon stays stale.
  Never read either line as a verdict on the seat - it says the restarter is not running, and what it would fail to restore is the NEXT absence; the paired `SEAT_ALARM:` line is the detection half and is repaired through `bin/fm-seat-alarm.sh`.
- `RESPAWNER_UNIT: the first-mate restart watch could not be armed on this home ...` - session start could not install that check at all, so the restart would stop with this session; run the named `bin/fm-seat-respawner-service.sh --arm` to see why and report the reason.
- `RESPAWNER_UNIT: failed to encode FM_HOME ...` - the home path could not be encoded into a unit instance name, so no per-home unit can be addressed; report the path.
...
- `SEAT_ALARM: <detail>` - nothing is watching whether this vessel still has a first mate, so its own absence would again be noticed only by the captain walking into it.
  `nothing is watching` or `could not be armed` means the watch was never installed or the arm failed; run `bin/fm-seat-alarm.sh --arm` and report the reason if it refuses.
  `has never completed a reading` or `has stopped running` means the watch exists but the monitoring service is not running it, which is a supervision fault rather than a seat one: repair it through the emitted supervision instructions, exactly as for a lapsed watcher.
  This line is never a verdict on the seat, and it cannot be, since a session reading it is itself the evidence a seat is present - it says the instrument is not reading, and what it would fail to report is the NEXT absence.
  Unlike every other alarm in this fleet, that report goes straight to the captain rather than through a wake, because firstmate is the subject of the reading; `docs/seat-absence.md` owns why, and `bin/fm-seat-alarm.sh --status` gives the current reading.
  A paired `RESPAWNER_UNIT:` line about the restart having stopped is the other half and is repaired the same way, through `bin/fm-seat-respawner-service.sh`.
