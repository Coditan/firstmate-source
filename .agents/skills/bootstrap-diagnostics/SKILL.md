...
- `LAVISH_ACCESS: <N> open review board link(s) still point at this machine only ...` - this vessel has a tailnet, so those boards could be reachable, but the links already handed over open nothing on the captain's own devices.
  ...
  Reopen each affected board with `bin/fm-lavish.sh <html-file>`, which reopens it on a link that reaches this vessel off this machine - bound on the tailnet address, or bound on loopback and published onto that address over `tailscale serve` where it cannot be bound - and prints the new link, then give the captain that new link if he was already sent the old one.
  ...
  The check is detect-only, stays silent unless this vessel's last recorded resolution says it has reach to offer, and repeats each session start until the affected sessions are reopened or ended.
...
- `DECISION_LEDGER: STUCK: this home's captain decision records could not be read: <cause>` - the audit refused instead of answering, and the cause is the reader's own: `could not read <data dir>` for record files it could not parse, `backlog parse produced no records at all ...` or `backlog parse produced invalid records ...` for a parse that produced nothing usable, and `could not classify this home's records ...` for a model it could not build.
  This is the one thing a silent session start would hide, because a home that cannot read its decisions otherwise looks exactly like a home holding none; repair the named file or its permissions, then re-run `bin/fm-decision-ledger.sh --audit` to see the findings it was withholding.
