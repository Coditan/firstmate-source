...
- `BACKLOG_STALE: task <id> has <fault>; fix: <command>` - a current durable backlog record has a mechanically stale dependency edge.
  Inspect the named record, run the exact printed `tasks-axi unblock` command to remove that edge, and add the intended existing blocker afterward only when the dangling id was a typo.
