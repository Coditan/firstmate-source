...
A `non-member-variant` record is the mirror of that one and the only row here that is not a member: the ruling is the member, retrofitted onto this chart by a member list out of a group the chart does not own, and the fold hung this record beneath it.
