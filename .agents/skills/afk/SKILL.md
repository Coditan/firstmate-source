...
- A message **without** the current operational prefix or a legacy bare marker, and **not** starting with `/afk` -> the captain is back.
  ...
  If it reports a firstmate-actionable `blocked:` event, remediate it immediately through the normal lifecycle, or explicitly reclassify it with a durable reason and close its decision key with `bin/fm-status.sh <status-file> resolved --key <key> "<reason>"`, then run `bin/fm-afk-return.sh check`.
...
The one captain-named deviation from the standing steering rules inside an away window is the omega protocol, which the `omega-protocol` skill owns in full; it is never in force from `/afk` alone.
