...
- **Event-driven, zero-token supervision** - a bash watcher sleeps on the fleet and a companion service delivers the wake, both outside the agent harness so neither can be killed with a session; verified primary harnesses also get a turn-end backstop that blocks or follows up on a blind stop when work is under way and supervision is not live; and if the first mate itself is the part that dies, that same watcher reports the absence to you directly, on a home with an outbound channel configured, and puts a seat back without you ([docs/seat-absence.md](docs/seat-absence.md)).
...
- [docs/codex-sandbox-gate-repo.md](docs/codex-sandbox-gate-repo.md) - why a Codex crewmate carries its project's no-mistakes gate repository as a writable root, the measurement showing one gate is enough for the push, and why the repository root and the daemon directory stay refused.
- [docs/codex-sandbox-unavailable.md](docs/codex-sandbox-unavailable.md) - what a Codex launch degrades to on a host whose kernel refuses to start a sandbox at all, how the host is probed, why the tracked Codex profile is never rewritten for it, and the measured evidence for both walls.
...
- [docs/merge-gate-audit.md](docs/merge-gate-audit.md) - how to audit GitHub merge gates across rulesets and classic branch protection, including the current registered merge-target gate reading.
...
- [docs/away-mode-approval-authority.md](docs/away-mode-approval-authority.md) - why away mode neither widens nor withdraws approval authority, the contradiction that wording resolves, and the reading of the captain's ruling that was deliberately rejected.
- [docs/captain-facing-readability.md](docs/captain-facing-readability.md) - the worked example behind the rule that a captain-facing message must be readable by someone who was not in the session, and why it shortens what he reads without narrowing what reaches him.
...
- [docs/seat-respawner.md](docs/seat-respawner.md) - how a dead primary seat is relaunched, how to declare an intentional stay-down, why manual close without that marker brings the seat back, and the terminal-hosted keeper that stands in for the unit in a home with no per-user service manager.
- [docs/seat-absence.md](docs/seat-absence.md) - what happens when the first mate itself is gone: how the absence reaches you with no session to route it, what supervises the restart once the seat cannot, and what neither half covers.
