...
- [docs/context-reset.md](docs/context-reset.md) - the stow-then-clear context ceiling: what the watcher measures, why it asks instead of acting while the captain is present, and every refusal the reset tool makes.
...
Third-party material: `/sea-chart` and `bin/fm-sea-chart.sh` amend the Wayfinder skill by Matt Pocock, from [`mattpocock/skills`](https://github.com/mattpocock/skills), used under the MIT licence.
[`docs/sea-chart-provenance.md`](docs/sea-chart-provenance.md) carries that copyright notice and licence text, along with what was kept, changed, and dropped.
