...
#
# <surface> is yes for every ordinary reading.
READINGS=()
...
# The cross-home arm guard: an armed check must land in the state directory
# of the home it bakes (bin/fm-check-lib.sh).
...
arm() {
  ...
  fm_check_arm_refuse fm-currency-round "$STATE" "$FM_HOME" || return 1
  desired=$(cat <<SHIM
#!/usr/bin/env bash
