...
# THIS ALARM BELONGS TO THE VESSEL IT WATCHES, AND TO NOTHING ELSE.
# A secondmate home has no outbound sender of its own - the Telegram
# configuration is deliberately not inherited - so every send from one would
# fail, and a failed send is never counted as a notification. The transition
# line would then print on every sweep and append a durable wake to a queue
# nobody is draining, for as long as the home exists, while addressing the
...
armed_diagnostic() {
  local mtime age
  # Nothing is armed here on purpose, so nothing is missing.
...
case "$MODE" in
  arm)
    ...
    # Say which of the two things happened. Reporting "armed" for a home this
    # deliberately does not arm would be the same overclaim the rest of this
    # file refuses: a line asserting a state that was not established.
    if fm_root_is_secondmate_home "$FM_HOME"; then
      printf 'not armed: %s watches the vessel it runs on, and this home is not it\n' "$(basename "$0")"
    else
      printf 'armed: %s\n' "$CHECK"
    fi
    exit 0 ;;
