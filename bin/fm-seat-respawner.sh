...
tmux_server_pid_from_endpoint() {
  local server
  server=$(kv_get "$(endpoint_file)" tmux-server 2>/dev/null || true)
  printf '%s' "${server##*,}"
}
...
# it alone. kv_get reads the first match, so a key is only ever marked once.
first_turn_mark() {  # <key>
  local tmp
...
deliver_first_turn() {
  ...
  server=$(kv_get "$FIRST_TURN" server 2>/dev/null || true)
  at=$(kv_get "$FIRST_TURN" at 2>/dev/null || true)
  submitted=$(kv_get "$FIRST_TURN" submitted 2>/dev/null || true)
...
emit_giveup_finding() {  # <key> <status-line> <launches> <holds> <pane> <ever-confirmed> <last-probe-rc>
  local key=$1 status_line=$2 launches=$3 holds=$4 pane=$5 confirmed=$6 probe=$7 out claim measurement refuted where seen
...
one_cycle() {
  ...
  if [ "$spent" -ge "$MAX_ATTEMPTS" ]; then
    ...
    probe=$(kv_get "$FIRST_TURN" pane-probe 2>/dev/null || true)
